...
calculateStatsFiveWords({required bool gameWon}) async {
  int gamesPlayed = 0;
  int gamesWon = 0;
  int winPercent = 0;
  int currentSteak = 0;
  ...
  final stats = await getStatsFiveWords();
  if(stats != null){
  ...
  prefs.setStringList('stats_five_words', [
...
Future<List<String>?> getStatsFiveWords() async {
  final prefs = await SharedPreferences.getInstance();
  final stats = prefs.getStringList('stats_five_words');
  if(stats != null){
    return stats;
  }else{
    return null;
